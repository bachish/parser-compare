...
@op(ins={"db_path": In(str), "errors_csv_path": In(str), "codes_db_path": In(str)}, out=Out(str))
def populate_file_errors(context: OpExecutionContext, db_path: str, errors_csv_path: str, codes_db_path: str) -> str:
    ...
    with open(errors_csv_path, newline='', encoding='utf-8') as csvfile:
        ...
        for i, row in enumerate(rows):
            ...
            cursor.execute(
                ...
                WHERE id_file = ? AND id_error_message = ?
                """,
                (id_file, id_error_message)
            )
            if cursor.fetchone():
                # context.log.debug(f"Error for file {id_file}, message {id_error_message} already exists, skipping")
                skipped_count += 1
                continue
            ...
            cursor.execute(
                """
                INSERT INTO javac_file_errors (id_file, id_error_message)
                VALUES (?, ?)
                """,
                (id_file, id_error_message)
            )
